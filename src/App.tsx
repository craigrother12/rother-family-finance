    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900">
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b">
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2"><div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[11px] bg-white">{cloudStatus==='synced' && <span className="text-emerald-700">☁️ Synced {lastSync}</span>}{cloudStatus==='syncing' && <span>⏳ Syncing...</span>}{cloudStatus==='error' && <span className="text-red-600">☁️ Error</span>}{cloudStatus==='offline' && <span>📴 Local</span>}</div><div className="hidden md:flex gap-2"><div className="px-3 py-1.5 rounded-full bg-slate-900 text-white text-xs">{fmt(netWorth)}</div><div className="px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[11px] border">{fmt(trueLiquidCash)}</div></div><button onClick={exportBackup} className="px-3 py-2 rounded-xl bg-emerald-600 text-white border text-xs font-semibold">Export</button><label className="px-3 py-2 rounded-xl bg-white border text-xs cursor-pointer">Import<input type="file" accept=".json" className="hidden" onChange={e=>{ const f=e.target.files?.[0]; if(f) importBackup(f); e.target.value=''; }}/></label><button onClick={manualSync} className="px-3 py-2 rounded-xl bg-slate-900 text-white border text-xs">Push</button><button onClick={pullFromCloud} className="px-3 py-2 rounded-xl bg-white border text-xs">Pull</button><button onClick={()=>{ try{ sessionStorage.removeItem('fam-auth'); }catch{} location.reload(); }} className="px-3 py-2 rounded-xl bg-white border text-xs">Lock</button><button onClick={resetToBlankSlate} className="px-3 py-2 rounded-xl bg-white border text-xs">Blank</button></div>
        </div>
